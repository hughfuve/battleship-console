...
char 	tmpBuffer[256];			//temp buffer for constructing display elements
	...
	// *******************************************************
		...
		switch (cmd){
			...
			case SERVICE:  // services all events for the main world 
				...
				//move (5,5);
				//printw("A KEY event was fired [%c]",world->key);
				//refresh();
			...
			case SERVICE:			
				...
				switch(world->key){
					...
					case (char) 0x0d:
							...
							//THEN TAKE A HIT FROM THE ENEMY
							numTargets=0;
							result=TRY_AGAIN;
										...
										switch(target->state){
											case NORTH:												
												while(result==TRY_AGAIN && target->changeDirection==false){
													...
													if(target->targetVectorY-steps > 0 && steps<partLengths[(int)targetType]){
											...
											case EAST:												
												while(result==TRY_AGAIN && target->changeDirection==false){
													...
													if(target->targetVectorX-steps >0 && steps<partLengths[(int)targetType]){
							...
							if(computer->lost>4){
								//sprintf(tmpBuffer,"Player WINS! ");
								//messageHandler(ADD,tmpBuffer);		
										
								//sprintf(tmpBuffer,"GAME OVER ");
								//messageHandler(ADD,tmpBuffer);
	...
	// *******************************************************
	bool target_CHANGE_DIRECTION(TARGET * target){               //manages direction changing for the targeting AI
	// *******************************************************
		switch(target->state){
			case NORTH:
	...
	//REF: http://www.cplusplus.com/forum/general/16335/
	...
	// *******************************************************	
	//REF: http://invisible-island.net/ncurses/ncurses-intro.html#using
	...
	// *******************************************************
	//REF: http://www.zedwood.com/article/cpp-str_replace-function
 	string& str_replace(const string &search, const string &replace, string &subject){
	// *******************************************************
		string buffer;
